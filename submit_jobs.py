...
def main(state, num_tunes, num_draws, run_elec, run_county, outdir):
    ...
    for county in counties:
        ...
        for elec in elecs:
            if run_elec:
                if elec != run_elec:
                    continue
            for groups in [["APAMINVAP"], ["AMINVAP"]]:
                ...
                output_folder = f"{outdir}/{state+county_id}/diagnostics"
                ...
                with open("job.sh", "w") as f:
                    ...
                    f.writelines(f"python ei.py -state {state} -elec {elec} {groups_arg} -num_tunes {num_tunes} -num_draws {num_draws} -outdir {outdir} {county_arg}\n")
                    f.writelines(f"python viz.py -state {state} -elec {elec} {groups_arg} -outdir {outdir} {county_arg}\n")
                    f.writelines(f"python summary.py -state {state} -elec {elec} {groups_arg} -outdir {outdir} {county_arg}\n")
