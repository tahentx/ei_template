...
def create_processed_df(state, elec, g, county, outdir):
    ...
    output_folder = f"{outdir}/{state+county_id}/final_inputs"
...
def run_ei(state, elec, g, num_tunes, num_draws, county, outdir):
    ...
    pareto_scale = 1000 
    pareto_shape = 200 
    ei = RowByColumnEI(model_name='multinomial-dirichlet-modified', pareto_scale=pareto_scale, pareto_shape=pareto_shape)
    ...
    print(f"Pareto scale: {pareto_scale}, Pareto shape: {pareto_shape}")
    county_id = '' if county is None else f'_{county.replace(" ", "")}'
    output_folder = f"{outdir}/{state+county_id}/ei"
    os.makedirs(output_folder, exist_ok=True)
...
def main(state, elec, g, num_tunes, num_draws, county, outdir):
