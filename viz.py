...
def plot_ER(state, election, groups, outdir):
    df = pd.read_csv(f"{outdir}/{state}/final_inputs/{state}_{election}_{'-'.join(groups)}.csv", index_col=0)
    ...
    os.makedirs(f"{outdir}/{state}/plots/ER", exist_ok=True)
    plt.savefig(f"{outdir}/{state}/plots/ER/{state}_{election}_scatter.png", dpi=600, bbox_inches='tight')
    plt.show()
    plt.close()
    return
...
def main(state, elec, g, outdir, county=None):
    ...
    output_folder = f"{outdir}/{state+county_id}/plots"
    os.makedirs(output_folder, exist_ok=True)

    plot_ER(state, elec, g, outdir)
